...
//               <input type="email" name="email" id="email" className="border border-gray-300 rounded-lg focus:border-green-500 w-full px-4 py-2" placeholder="name@company.com" required="" />
//             </div>
//             <div className="mb-4">
//               <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-700">Password</label>
//               <input type="password" name="password" id="password" className="border border-gray-300 rounded-lg focus:border-green-500 w-full px-4 py-2" placeholder="••••••••" required="" />
//             </div>
...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_TIMEKEEPR_API}client/login`, {
                method: 'POST',
            ...
            if (data.auth) {
                document.cookie = `token=${data.token}; path=/`; // set the cookie manually
